...
设置后，配置、会话、日志、OAuth 凭据等**全部**数据都会落到新路径下。`KIMI_CODE_HOME` 的完整说明见[环境变量](./environment-variables.md)。
...
- **`config.toml`**：主运行时配置，存放供应商、模型、循环控制等用户级设置。详见[配置文件](./configuration-files.md)。
...
- [配置文件](./configuration-files.md) — `config.toml` 各字段的完整说明
- [环境变量](./environment-variables.md) — `KIMI_CODE_HOME` 等路径变量的详细用法
