...
大型调研可以让主 Agent 派发**子 Agent** 并行处理子任务，详见 [Agent 与子 Agent](./customization/sub-agents.md)。
...
对于确定安全的批处理任务，可以用 `--yolo` 或 `/yolo` 跳过审批，也可以在[配置文件](./configuration/configuration-files.md#permission)里给特定工具预置白名单规则。
...
- [Agent 与子 Agent](./customization/sub-agents.md) — 如何让 Agent 派发子任务并行处理
