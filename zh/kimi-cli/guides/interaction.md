...
斜杠命令是以 `/` 开头的特殊指令，用于执行 Kimi CLI 的内置功能，如 `/help`、`/setup`、`/sessions` 等。输入 `/` 后会自动显示可用命令列表。完整的斜杠命令列表请参考 [斜杠命令参考](https://moonshotai.github.io/kimi-cli/zh/reference/slash-commands.html)。
