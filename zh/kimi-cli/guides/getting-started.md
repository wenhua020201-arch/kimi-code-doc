...
- **[交互式命令行（`kimi`）](https://moonshotai.github.io/kimi-cli/zh/reference/kimi-command.html)**：在终端中以 Shell 方式与 AI 对话，支持自然语言描述任务或直接执行 Shell 命令
- **[浏览器界面（`kimi web`）](https://moonshotai.github.io/kimi-cli/zh/reference/kimi-web.html)**：在本地浏览器中打开图形界面，支持会话管理、文件引用、代码高亮等
- **[Agent 集成（`kimi acp`）](https://moonshotai.github.io/kimi-cli/zh/reference/kimi-acp.html)**：以服务方式运行，通过 [Agent Client Protocol] 集成到 [IDE](./ides.html) 和其他本地 Agent 客户端中
...
输入 `/help` 可以查看所有可用的 [斜杠命令](https://moonshotai.github.io/kimi-cli/zh/reference/slash-commands.html) 和使用提示。
