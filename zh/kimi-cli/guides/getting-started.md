...
Kimi CLI 在终端中提供类似 Shell 的交互体验，你可以用自然语言描述需求，也可以随时切换到 Shell 模式直接执行命令。除了终端使用，Kimi CLI 还支持通过 [Agent Client Protocol] 集成到 [IDE](./ides.html) 和其他本地 Agent 客户端中。
...
配置完成后，Kimi CLI 会自动保存设置并重新加载。详见 [平台与模型](https://moonshotai.github.io/kimi-cli/zh/configuration/providers.html)。
