...
|--------------|-----------|---------|
| `The API Key appears to be invalid or may have expired` | 401 | [认证错误](#认证错误) |
| `Invalid Authentication` | 401 | [认证错误](#认证错误) |
| `unable to verify your membership benefits` | 402 | [会员权益异常](#会员权益异常) |
| `Kimi For Coding is currently only available for Coding Agents` | 403 | [权限错误](#权限错误) |
| `You've reached your usage limit for this billing cycle` | 403 | [权限错误](#权限错误) |
| `Access terminated` | 403 | [权限错误](#权限错误) |
| `The engine is currently overloaded` | 429 | [限流与配额](#限流与配额) |
| `We're receiving too many requests` | 429 | [限流与配额](#限流与配额) |
| `You've reached your usage limit for this period` | 429 | [限流与配额](#限流与配额) |
| `You've reached kimi monthly usage limit` | 429 | [限流与配额](#限流与配额) |
| `total message size N exceeds limit 2097152` | 400 | [请求格式错误](#请求格式错误) |
| `Your request exceeded model token limit: 262144` | 400 | [请求格式错误](#请求格式错误) |
| `thinking is enabled but reasoning_content is missing` | 400 | [请求格式错误](#请求格式错误) |
| `unsupported image url` | 400 | [请求格式错误](#请求格式错误) |
| `function name ... is duplicated` | 400 | [请求格式错误](#请求格式错误) |
| `The request was rejected because it was considered high risk` | 400 | [请求格式错误](#请求格式错误) |
| `Not found the model kimi-for-coding or Permission denied` | 404 | [资源未找到](#资源未找到) |
| `method not found` | 404 | [资源未找到](#资源未找到) |
| `bot_id ... value does not match id_kinds` | 500 | [服务端内部错误](#服务端内部错误) |
| `failed to connect to ... database=membership_` | 500 | [服务端内部错误](#服务端内部错误) |
| `FATAL: terminating connection due to administrator command` | 500 | [服务端内部错误](#服务端内部错误) |
| `failed to evaluate rate limit script` | 500 | [服务端内部错误](#服务端内部错误) |
| `i/o timeout` / `conn closed` / `bad connection` | 500 | [服务端内部错误](#服务端内部错误) |
| `503 Service Unavailable` / `504 Gateway Timeout` / `502 Bad Gateway` | 500 | [服务端内部错误](#服务端内部错误) |
| `未找到该账号` / `该账号已被禁用` / `已被禁言` | 500 | [服务端内部错误](#服务端内部错误) |
| `context canceled` | 499 | [工具调用错误](#工具调用错误) |
| `url2text` / `spider checkUrl failed` / `invalid html` | 500 | [工具调用错误](#工具调用错误) |
| `image_url:moderation request error` | 500 | [工具调用错误](#工具调用错误) |
| `We consider the current URL poses a security risk` | 403 | [工具调用错误](#工具调用错误) |
| `invalid_url: The provided URL is invalid` | 400 | [工具调用错误](#工具调用错误) |
