...
Slash commands are special instructions starting with `/`, used to execute Kimi Code CLI's built-in features, such as `/help`, `/login`, `/sessions`, etc. After typing `/`, a list of available commands will automatically appear. For the complete list of slash commands, see the [slash commands reference](https://moonshotai.github.io/kimi-cli/en/reference/slash-commands.html).
...
You can also set `default_yolo = true` in the config file to enable YOLO mode by default on every startup. See [Configuration files](https://moonshotai.github.io/kimi-cli/en/configuration/config-files.html).
