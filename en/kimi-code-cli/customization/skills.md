...
You can also specify additional skills directories with the `--skills-dir` parameter. This parameter can be specified multiple times, and the directories override the auto-discovered user and project directories:

```sh
kimi --skills-dir /path/to/my-skills --skills-dir /path/to/more-skills
```

> Skills paths are independent of `KIMI_SHARE_DIR`. `KIMI_SHARE_DIR` customizes the storage location for configuration, sessions, logs, and other runtime data, but does not affect Skills search paths. Skills are cross-tool shared capability extensions (compatible with Kimi CLI, Claude, Codex, and others), which is a different type of data from application runtime data. To specify custom skills paths, use the `--skills-dir` parameter.
