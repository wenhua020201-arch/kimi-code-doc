...
> You can customize the share directory path by setting the `KIMI_SHARE_DIR` environment variable.
>
> Note: `KIMI_SHARE_DIR` only affects the storage location of the runtime data listed above, not Agent Skills search paths. Skills, as cross-tool shared capability extensions, are a different type of data from runtime data.
...
Main configuration file, stores providers, models, services, and runtime parameters. See [Config Files](/en/kimi-code-cli/configuration/configuration-files) for details.

You can specify a configuration file at a different location with the `--config-file` parameter.
...
This file is automatically managed by Kimi Code CLI and typically does not need manual editing.

### mcp.json

MCP server configuration file, stores MCP servers added via the `kimi mcp add` command. See [MCP](/en/kimi-code-cli/customization/mcp) for details.
...
Context history file, stores the session's full context in JSON Lines (JSONL) format. The first line is a system prompt record (_system_prompt), followed by messages (user input, model response, tool calls, etc.) or internal records (checkpoints, token usage, etc.).
...
Plan mode plan files are stored in the `~/.kimi/plans/` directory. Each Plan session corresponds to a randomly named Markdown file (i.e., `<slug>.md`).

plan_slug is saved in state.json, so the same plan file is resumed after a process restart. Use the `/plan clear` command to delete the current Plan session's plan file.
...
Runtime logs are stored in `~/.kimi/logs/kimi.log`. The default log level is INFO; use the `--debug` parameter to enable TRACE level.
...
| Clear sessions for a specific working directory | Use `/sessions` in Shell mode to view and delete |
